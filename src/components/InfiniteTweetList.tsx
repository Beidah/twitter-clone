...
export default function InfiniteTweetList({ tweets, isError, isLoading, hasMore = false, fetchNewTweets }: InfiniteTweetListProps) {
...
function TweetCard({ id, user, content, createdAt, likeCount, likedByMe }: Tweet) {
  ...
  const toggleLike = api.tweet.toggleLike.useMutation({
    onSuccess: ({ addedLike }) => {
      const updateData: Parameters<typeof trpcContext.tweet.infiniteFeed.setInfiniteData>[1]
        = (oldData) => {
...
function HeartButton({ isLoading, onClick, likedByMe, likeCount }: HeartButtonProps) {
  ...
  return (
    <button
      disabled={ isLoading }
      onClick={ onClick }
      className={ `group -ml-2 items-center gap-1 self-start flex transistion-colors duration-200 ${likedByMe
        ? "text-red-500"
      ...
      <IconHoverEffect red>
        <HeartIcon
          className={ `transition-colors duration-200 ${likedByMe
            ? "fill-red-500"
            : "fill-gray-500 group-hover:fill-redd-500 group-focus-visible:fill-red-500"
            }`
          }
        />
      </IconHoverEffect>
